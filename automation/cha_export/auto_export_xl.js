...
async function generateOutputs(page, baseName, config, fileInfo = null) {
  ...
  if (config.useGoogleDrive && config.googleOutputFolderId && downloadPath && fs.existsSync(downloadPath)) {
    ...
    try {
      const { initGoogleDrive } = await import('./google_drive.js');
      const driveClient = await initGoogleDrive();
      const fileMetadata = await driveClient.uploadFile(downloadPath, config.googleOutputFolderId);
      uploadResult = {
...
async function runBatch() {
  ...
  // ── Emit total count for dashboard progress tracking ──────────────
  console.log(`[PROGRESS] BATCH_TOTAL=${files.length}`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    console.log(`[PROGRESS] FILE_START=${fileName}`);
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`[PROGRESS] FILE_DONE=${fileName} STATUS=success DURATION=${duration}s`);
      } else {
        ...
        console.log(`[PROGRESS] FILE_DONE=${fileName} STATUS=failed DURATION=${duration}s`);
      }
    } catch (err) {
