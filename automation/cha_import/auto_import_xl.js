/**
 * ============================================================================
 *  auto_import_xl.js — Batch CHA Import Automation (v1)
 * ============================================================================
 *
 *  Automates the Xemi CHA Import workflow for up to 500+ Excel files.
 *
 *  Features:
 ...
 *
 *  Usage:
 *    node auto_import_xl.js                      # Batch — all files
 *    node auto_import_xl.js --file=myfile.xlsx    # Single file
 *    node auto_import_xl.js --help                # Show usage
 *
 *  Environment Variables (all optional, set in ../.env):
 *    BATCH_SIZE       — Max files per run (0 = unlimited, default: 0)
 *    INPUT_DIR        — Input folder (default: ./input/input_excel)
 *    OUTPUT_SB_DIR    — SB output folder (default: ./output/output_sb)
...
// Try to find .env in current, parent, or grandparent (project root)
const envPath = [
...
import { login, ensureSession } from '../cha_export/user_login.js';
import { convertSingleExcelToJson } from '../cha_export/xl_to_json.js';
import {
  retry,
  safeClick,
  safeFill,
  ...
  BatchLogger,
  ProgressBar,
} from '../cha_export/utils.js';
import {
  initGoogleDrive,
  fetchFilesFromGoogleDrive,
  uploadFilesToGoogleDrive,
  printSetupInstructions as printGoogleDriveSetup,
} from '../cha_export/google_drive.js';

// ─── Configuration ──────────────────────────────────────────────────────────

const CONFIG = {
  inputDir:      process.env.INPUT_DIR        || './input/input_excel',
  jsonDir:       './input/input_json',
  outputSbDir:   process.env.OUTPUT_SB_DIR    || './output/output_sb',
...
function parseCLIArgs() {
  ...
  if (process.argv.includes('--help')) {
    console.log(`
Usage:
  node auto_import_xl.js                   Batch mode — process all Excel files
  node auto_import_xl.js --file=abc.xlsx   Single file mode

Environment Variables:
  BATCH_SIZE              Max files per run (0 = all)
  INPUT_DIR               Input folder path (default: ./input/input_excel)
  OUTPUT_SB_DIR           SB output folder (default: ./output/output_sb)
...
Run 'node auto_import_xl.js --google-setup' for Google Drive setup instructions.
    `);
    process.exit(0);
  }
...
async function fetchFilesFromGoogleDriveHelper() {
  const { initGoogleDrive, fetchFilesFromGoogleDrive } = await import('../cha_export/google_drive.js');
...
/**
 * Process a single Excel file through the full CHA Import workflow.
 *
...
async function processFile(page, fileEntry, config) {
  ...
  for (const row of rows) {
    ...
    const importer = fieldValidator.check(fileName, row, 'Importer Name', [
      'Importer Name', 'Importer', 'Importer_Name',
    ]);

    if (!importer) {
      console.log(`    ⚠️ Missing importer field. Skipping row.`);
      continue;
    }
    ...
    console.log(`    🚀 Creating job for: ${importer}`);

    // ── Navigate to Import CCM ────────────────────────────────────────
    try {
      await retry(async () => {
        await page.goto(`${process.env.BASE_URL}/import-ccm`, {
          waitUntil: 'networkidle',
          timeout: 30000,
        });
      }, { retries: 2, label: 'Navigate to import-ccm' });
    } catch (err) {
    ...
    try {
      ...
      importerFound = await trySearch(importer);
      if (!importerFound) {
        const shortName = importer.split(' ').slice(0, 2).join(' ');
        console.log(`    ⚠️ Full name not found, trying: ${shortName}`);
        importerFound = await trySearch(shortName);
      }

      if (importerFound) {
        await safeClick(page.locator('nz-option-item').first());
      } else {
        console.log(`    ❌ Importer not found in dropdown: ${importer}. Skipping.`);
        continue;
      }
    } catch (err) {
      stepErrors.push(`Dropdown error for "${importer}": ${err.message}`);
    ...
    console.log(`    ✅ Job complete for: ${importer}`);
  }
...
async function generateOutputs(page, baseName, config, fileInfo = null) {
  ...
  if (config.useGoogleDrive && config.googleOutputFolderId && downloadPath && fs.existsSync(downloadPath)) {
    ...
    try {
      const { initGoogleDrive, uploadFilesToGoogleDrive } = await import('../cha_export/google_drive.js');
      const driveClient = await initGoogleDrive();
...
async function runBatch() {
  ...
  // ── Write logs ─────────────────────────────────────────────────────
  logger.writeErrorLog('./output/error_log.txt');
  fieldValidator.writeLog('./output/missing_fields_log.json');
